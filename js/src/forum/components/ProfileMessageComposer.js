...
let ProfileMessageComposerClass = null;

export default async function getProfileMessageComposer() {
  if (ProfileMessageComposerClass) return ProfileMessageComposerClass;

  const ComposerBody = await flarum.reg.asyncModuleImport('flarum/forum/components/ComposerBody');

  ProfileMessageComposerClass = class ProfileMessageComposer extends ComposerBody {
    ...
    headerItems() {
      ...
      items.add('title', <h3><Icon name={iconName} />{' '}{heading}</h3>);
  ...
  return ProfileMessageComposerClass;
}
